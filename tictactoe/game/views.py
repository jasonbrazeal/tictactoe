...
def home(request):
    ...
    if request.session.get('has_game', False):
        g = Game.objects.filter(session_id=request.session.session_key).order_by('-date_created')[0]
        board = g.get_board()
...
def setup(request):
    ...
    g = Game.objects.filter(session_id=request.session.session_key)
    if not g:
      g = Game(session_id=request.session.session_key, player_human=player_human, player_AI=player_AI)
      g.save()
